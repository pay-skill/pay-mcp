...
Before the first payment, the CLI must be installed and initialized.
...
If `cargo` is not available, check for the binary at `~/bin/pay`.
If neither exists, report to the operator with install options:
- `cargo install pay-cli`
...
```bash
test -f ~/.pay/config.toml || pay init
```

`pay init` generates a keypair (k256, AES-256-GCM encrypted storage),
...
Send via whatever communication channel is active:
- Telegram bridge (if away mode is on)
- Terminal output (always)
- Any other comm channel the operator has configured

Do not rely on terminal alone — the operator may be AFK.

### 3. Poll briefly

After sending the link, poll for a few minutes:
