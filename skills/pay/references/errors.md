...
|-------|---------|----------|
| `INSUFFICIENT_BALANCE` | Wallet or tab is empty | `pay fund` → send link → wait for funding → retry |
...
3. Send link via Telegram / active comm channel
4. Poll briefly (1-2 minutes, short intervals)
5. If funded → retry payment
...
2. Run `pay tab topup <id> <amount>` (same size as original tab)
